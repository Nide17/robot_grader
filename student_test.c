...
void usage(const char *argv0)
{
  ...
  fprintf(stderr, "the number of tests d and also possibly a memory leak notice\n\n");
...
int main(int argc, char *argv[])
{
  ...
  if (getenv(ENV_TESTS_d))
    tests_d = atoi(getenv(ENV_TESTS_d));
  ...
  if (tests_d < 0 || 10 < tests_d) {
    fprintf(stderr, "Illegal value for tests_d: %d\n", tests_d);
    usage(argv[0]);
    return 1;
  }
